...
export const removeDollars = (amounts: string[]): number[] => {
    ...
    return removedSigns.map((amount: string): number =>
...
export function countShortWords(words: string[]): number {
    return words.reduce(
        (shortWordsCount: number, word: string) =>
            word.length < 4 ? shortWordsCount + 1 : shortWordsCount,
        0
    );
}
...
export function injectPositive(values: number[]): number[] {
    const firstNegativeIndex: number = values.findIndex(
        (number: number): boolean => number < 0
    );

    const findSum = (numbers: number[]): number => {
        return numbers.reduce((sum: number, number: number) => sum + number, 0);
    };

    if (firstNegativeIndex === -1) {
        return [...values, findSum(values)];
    } else {
        const valuesDuplicate = [...values];
        valuesDuplicate.splice(
            firstNegativeIndex + 1,
            0,
            findSum(values.slice(0, firstNegativeIndex))
        );
